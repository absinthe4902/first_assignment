...
public class PrintActivity extends AppCompatActivity {
    ...
    public void sendPost(JsonRequest body) {
        ...
        */
         ...
         */
        myDataServie.getRetroResponse(body).enqueue(new Callback<RetroResponse>() {
            ...
            public void onResponse(@NonNull Call<RetroResponse> call, @NonNull Response<RetroResponse> response) {
                    if (response.isSuccessful()) {
                        ...
                        tvSession.setText(String.format("%s %s", getResources().getString(R.string.text_session), response.body().getSessionKey()));
                        tvUserId.setText(String.format("%s %s", getResources().getString(R.string.text_user_id), String.valueOf(response.body().getUserId()))); //이런 소괄호 난리 정말 피하고 싶었는데 String.format에 %s %d를 나란히 쓰면 오류나서 숫자를 문자열로 바꾸고 사용.
                        tvUserType.setText(String.format("%s %s", getResources().getString(R.string.text_user_type), response.body().getUserType()));
                        tvStartMode.setText(String.format("%s %s", getResources().getString(R.string.text_start_mode), response.body().getStartMode()));
                        tvDevices.setText(String.format(("%s %s"), getResources().getString(R.string.text_devices), response.body().getDevicesList()));

                        response.body().responseDebug(); // 로그 찍어보기~
